...
function draw() {
  ...
  if ((trex.x%200===0))//||Math.round(trex.position.x%400===3)||Math.round(trex.position.x%400===4)||Math.round(trex.position.x%400===5)||Math.round(trex.position.x%400===2))
  {
    ground.x = trex.x;
  ...
  if (gameState===1){
    ...
    //trex.velocityX = (6 + 3*score/200);
    //console.log(Math.round(camera.position.x%400))
    
    score = score + Math.round(camera.position.x/1000);
